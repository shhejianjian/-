...
@interface XHGuardViewController ()<UITextViewDelegate,UIAlertViewDelegate>
...
@implementation XHGuardViewController
...
- (void)setupNav
{
    self.navigationItem.title = @"我要保安";
...
- (void)finish
{
    ...
    if (self.instrumentTextView.text.length != 0) {
        ...
        [SVProgressHUD showErrorWithStatus:@"请填写保安说明"];
    }
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        [BSHttpTool post:XHGuardUrl  params:params success:^(id json) {
