...
@interface XHDoctorViewController ()<UITextViewDelegate,UIAlertViewDelegate>
...
@implementation XHDoctorViewController
...
-(void)setupNav
{
    self.navigationItem.title = @"我要导医";
...
- (void)finish
{
    ...
    if (self.instrumentTextView.text.length != 0) {
        ...
        [SVProgressHUD showErrorWithStatus:@"请填写导医说明"];
    }
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        [BSHttpTool post:XHDoctorUrl  params:params success:^(id json) {
